...
@php
   ...
   $coupons = App\Models\Coupon::where('client_id', $client->id)->where('status', '1')->limit(3)->first();
                   ...
                   <div class="tab-pane fade show active" id="pills-order-online" role="tabpanel" aria-labelledby="pills-order-online-tab">
                     ...
                     <div id="#menu" class="bg-white rounded shadow-sm p-4 mb-4 explore-outlets"> 
                                 ...
                                 <div class="item">
                                    <div class="mall-category-item">
                                       <a href="#">
                                          ...
                                          @endif
                                          <span class="float-right"><a class="btn btn-outline-secondary btn-sm" href="{{ route('add_to_cart', $populer->id) }}">ADD</a></span>
                                       </a>
                                    </div>
                                 </div> 
                      ...
                      <div class="row">
                        ...
                        @php
                           $best_sellers = App\Models\Product::where('status', 1)->where('client_id', $client->id)->where('best_seller',1)->orderBy('id','desc')->limit(3)->get();
                        @endphp
                           ...
                           <div class="col-md-4 col-sm-6 mb-4">
                              <div class="list-card bg-white h-100 rounded overflow-hidden position-relative shadow-sm">
                                 <div class="list-card-image">
                                    ...
                                    <a href="#">
                                    <img src="{{ asset($best_seller->image) }}" class="img-fluid item-img">
                                    </a>
                                 </div>
                                 <div class="p-3 position-relative">
                                    <div class="list-card-body">
                                       <h6 class="mb-1"><a href="#" class="text-black">{{ $best_seller->name }}</a></h6>
                                       <p class="text-gray mb-2">{{ $best_seller['city']['city_name'] }}</p>
                                       <p class="text-gray time mb-0">
                                          @if ($best_seller->discount_price == NULL) 
                                                <a class="btn btn-link btn-sm text-black" href="#"> 
                                                   ${{ $best_seller->price}}
                                                </a>  
                                          @else
                                                $<del>{{$best_seller->price}}</del>  
                                                <a class="btn btn-link btn-sm text-black" href="#"> 
                                                   ${{ $best_seller->discount_price}}
                                                </a>  
                                          @endif
                                          <span class="float-right"> 
                                             <a class="btn btn-outline-secondary btn-sm" href="{{ route('add_to_cart', $best_seller->id) }}">ADD</a>
                                          </span>
                                       </p>
                                    </div>
                                 </div>
                                 ...
                                 <div class="bg-white rounded border shadow-sm mb-4">
                                    ...
                                    @foreach ($menu->products as $product)
                                       ...
                                       <div class="menu-list p-3 border-bottom">
                                          <a class="btn btn-outline-secondary btn-sm  float-right" href="{{ route('add_to_cart', $menu->id) }}">ADD</a>
                                          <div class="media">
